...
def query_gpt(text_list, api_key, max_tokens=30, temperature=0, max_try_num=10, model="gpt-3.5-turbo-0125"):
    ...
    for prompt in tqdm(text_list):
        ...
        while curr_try_num < max_try_num:
            try:
                ...
                print(f"Invalid request for prompt: {prompt}. Error: {e}")
                result_list.append(-1)  # Log the failure but continue
                break  # Exit loop since this error is likely non-recoverable
            except Exception as e:
                print(f"Error for prompt: {prompt}, Attempt {curr_try_num + 1}/{max_try_num}. Error: {e}")
                curr_try_num += 1
                if curr_try_num >= max_try_num:
                    result_list.append(-1)  # Append failure after max attempts
